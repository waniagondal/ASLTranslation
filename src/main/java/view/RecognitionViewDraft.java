...
public class RecognitionViewDraft {
    ...
    public RecognitionViewDraft(PredictionInterface predictor) {
        this.predictor = predictor;
        this.transcriptionField = new JTextField(50);
        initializeUI();
    }
    ...
    private void updatePrediction() {
        try {
            String prediction = predictor.getPrediction();
            transcriptionField.setText(prediction);
        } catch (IOException | InterruptedException e) {
            transcriptionField.setText("Error fetching prediction.");
            e.printStackTrace();
        }
    }
}
